function runTriplePortExperiment_laser_centerAndSides(varargin)
...
% set up center laser stim for first trial
if p.centerPokeTrigger
    if rand <= p.centerLaserStimProb
       activateCenterLaserStim();
    else
...
elseif portID == rightPort.portID || portID == leftPort.portID
    ...
    if pokeHistory(pokeCount-1).isTRIAL == 1
        ...
        if timeSinceLastPoke <= p.centerPokeRewardWindow
            ...
            % The previous command used to be called after a delay. I don't think that's
            % necessary so I removed it.     OM 2021-04-09
...
function activateSideLaserStim()
...
function activateCenterLaserStim()
...
if p.centerPokeTrigger % if we're in centerPokeTrigger mode
    ...
    % when we leave the side port, let's make the choice if the center port
    % will activated the laser
%     if portID == rightPort.portID || portID == leftPort.portID
        %add logic for laser stimulation on the next entry to a center port
        disp('    Randomizing for center laser activation')
    if rand <= p.centerLaserStimProb
       activateCenterLaserStim();
    else
       deactivateCenterLaserStim();
